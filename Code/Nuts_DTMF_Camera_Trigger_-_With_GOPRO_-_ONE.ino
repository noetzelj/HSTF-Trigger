//Global variables-----------------------------------------------------------------------------------------
 ...
 const int STQ = 13;        // Attach DTMF Module STQ Pin to Arduino Digital Pin 2
 const int Q4 = 12;        // Attach DTMF Module Q4  Pin to Arduino Digital Pin 4
 const int Q3 = 11;        // Attach DTMF Module Q3  Pin to Arduino Digital Pin 5
 const int Q2 = 10;        // Attach DTMF Module Q2  Pin to Arduino Digital Pin 6
 const int Q1 = 9;        // Attach DTMF Module Q1  Pin to Arduino Digital Pin 7

 const int REMOTE_ID = 1; //define the remote ID here.....
...
// initialize digital pin 6 as GOPRO camera trigger
  pinMode(6, OUTPUT);
  digitalWrite(8, HIGH);


  // initialize digital pin 8 as WIRED camera trigger
  pinMode(8, OUTPUT);
  digitalWrite(8, LOW);
  ...
  //pin 4 controls PTT - the RED NC part
  pinMode(4, OUTPUT);
  digitalWrite(4, LOW);
...
void loop() {
  if(digitalRead(STQ)==HIGH){       //When a DTMF tone is detected, STQ will read HIGH for the duration of the tone.
    ...
    if (DTMFread==REMOTE_ID) {
      digitalWrite(8, HIGH);   //Fire the camera trigger
      digitalWrite(6, LOW);   //Fire the GoPro trigger
    }
  }
  else {
    digitalWrite(8, LOW);  //stop firing the trigger...
    digitalWrite(6, HIGH);  //stop firing the GOPRO trigger...
  if (DTMFread==REMOTE_ID){
            //open PTT
